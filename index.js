...
const { color, bgcolor } = require('./A187/color')
const { bahasa } = require('./A187/bahasa')
const { negara } = require('./A187/kodenegara')
const { virtex } = require('./A187/virtex')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./A187/functions')
const { fetchJson } = require('./A187/fetcher')
const { recognize } = require('./A187/ocr')
const { ind } = require('./A187')
const { help } = require('./A187/help')
const welkom = JSON.parse(fs.readFileSync('./A187/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./A187/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./A187/simi.json'))
const _leveling = JSON.parse(fs.readFileSync('./A187/leveling.json'))
const _level = JSON.parse(fs.readFileSync('./A187/level.json'))
const event = JSON.parse(fs.readFileSync('./A187/event.json'))
const _limit = JSON.parse(fs.readFileSync('./A187/limit.json'))
const uang = JSON.parse(fs.readFileSync('./A187/uang.json'))
const _registered = JSON.parse(fs.readFileSync('./A187/registered.json'))
const {WAConnection,MessageType,Presence,Mimetype,GroupSettingChange,MessageOptions,WALocationMessage,WA_MESSAGE_STUB_TYPES,ReconnectMode,ProxyAgent,waChatKey,mentionedJid,processTime,} = require('@adiwajshing/baileys')
let {botName,ownerName,BarBarKey,} = ('./A187/setting')
cr = '*MADE BY A187*'
ban = []
prefix = '!'
blocked = []
...
fs.writeFileSync('./A187/level.json', JSON.stringify(_level))
}
}
...
fs.writeFileSync('./A187/registered.json', JSON.stringify(_registered))
}
...
uang.push(obj)
fs.writeFileSync('./A187/uang.json', JSON.stringify(uang))
}
...
fs.writeFileSync('./A187/limit.json', JSON.stringify(_limit))
}
}
...
fs.existsSync('./A187.json') && client.loadAuthInfo('./A187.json')
...
teks = `Halo Member Baru @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}* \nSemoga betah disini dan jangan lupa intro dulu ya`
...
levelon: '*Successfully activated leveling💡*',
leveloff: '*Successfully disable leveling💡*',
levelnoton: '*Sorry, Leveling not aktif‼️*',
levelnol: '*Sorry, brothers level is still 0😩*',
error: {
stick: '*Failed, an error occurred while converting the image to a sticker‼️*',
Iv: 'Link not valid🙏'
},
only: {
benned: '*[❗] You To The Band Please Contact The Owner To Open Your Band*',
...
const NomerOwner = '6289524664142@s.whatsapp.net'
...
fs.writeFileSync('./A187/banned.json', JSON.stringify(ban))
...
tels = body.slice(8)
if (!isRegister) return reply(mess.only.daftarB)
if (isBanned) return reply(mess.only.benned) // Follow @_sadboy.ig 
if (isLimit(sender)) return reply(ind.limitend(pusname))
...
case 'ownergrup':
case 'ownergroup':
client.updatePresence(from, Presence.composing) // Follow @_sadboy.ig 
options = {
...
reply(mess.wait)
await ffmpeg(`./${media}`)
.inputFormat(media.split('.')[1]) // Follow @_sadboy.ig 
.on('start', function (cmd) {
console.log(`Started : ${cmd}`)
})
.on('error', function (err) {
...
welkom.push(from)
fs.writeFileSync('./A187/welkom.json', JSON.stringify(welkom))
...
if (!isRegister) return reply(mess.only.daftarB) // Follow @_sadboy.ig 
if (isBanned) return reply(mess.only.benned)
if (isLimit(sender)) return reply(ind.limitend(pusname))
tels = body.slice(7)
...
buffer = await getBuffer(anu.result)
client.sendMessage(from, buffer, image, {quoted: mek}) // Follow @_sadboy.ig 
await limitAdd(sender)
break
case 'nsfw':
if (!isGroup) return reply(mess.only.group)
...
nsfw.push(from)
fs.writeFileSync('./A187/nsfw.json', JSON.stringify(nsfw))
...
fs.writeFileSync('./A187/leveling.json', JSON.stringify(_leveling))
reply(mess.levelon)
} else if (args[0] === '0') {
_leveling.splice(groupId, 1)
fs.writeFileSync('./A187/leveling.json', JSON.stringify(_leveling)) // Follow @_sadboy.ig 
reply(mess.leveloff)
} else {
...
console.log(`*Error* :`, color(e,'red'))// Follow @_sadboy.ig 
reply('❌ *ERROR* ❌')
}
break
case 'ping':
...
break
case 'tiktok':
if (args.length < 1) return reply('Urlnya mana um?') // Follow @_sadboy.ig 
if (!isRegister) return reply(mess.only.daftarB)
if (isBanned) return reply(mess.only.benned)
if (isLimit(sender)) return reply(ind.limitend(pusname))
...
break
case 'simih':
if (!isGroup) return reply(mess.only.group) // Follow @_sadboy.ig 
if (!isGroupAdmins) return reply(mess.only.admin)
if (args.length < 1) return reply('ketik 1 untuk mengaktifkan fitur')
if (Number(args[0]) === 1) {
...
break
case 'translate':
var gl = body.slice(10) // Follow @_sadboy.ig 
var lg = gh.split("|")[0];
...
event.push(from)
fs.writeFileSync('./A187/event.json', JSON.stringify(event))
